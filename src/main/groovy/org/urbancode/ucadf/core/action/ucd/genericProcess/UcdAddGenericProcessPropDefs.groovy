/**
 * This action adds a generic process property definitions.
 */
...
class UcdAddGenericProcessPropDefs extends UcAdfAction {
	// Action properties.
	...
	public Object run() {
		...
		for (propDef in propDefs) {
			...
			if (existingUcdPropDef && failIfExists) {
				throw new UcAdfInvalidValueException("Process [$process] property name [${propDef.getName()}] already exists.")
			}
	
			if (existingUcdPropDef) {
				logVerbose("Process [$process] property definition [${propDef.getName()}] already exists.")
			} else {
				...
				Map requestMap = propDef.deriveRequestMap(ucdGenericProcess)
				...
				if (response.getStatus() == 200) {
					logVerbose("Process [$process] property definition [${propDef.getName()}] added.")
				} else {
