	...
	public Object run() {
		...
		for (teamSecurity in teams) {
			...
			if (response.getStatus() == 204) {
				logInfo("Application [$application] removed from team [$team]subtype [$subtype].")
	        } else if (response.getStatus() == 404) {
	            logInfo("Application [$application] team [$team]subtype [$subtype] not found.")
			} else if (response.getStatus() == 500) {
				logInfo("Ignoring status ${response.getStatus()} and assuming application [$application] team [$team]subtype [$subtype] not found.")
			} else {
